...
def ask_gemini(query, context, max_output_tokens=1000):
...
def query_rag_pipeline(query, faiss_index_path="Data/faiss_index"):
    index, texts = load_faiss(faiss_index_path)  # Load FAISS
    query_embedding = get_query_embedding(query)  # Convert query to vector
    retrieved_chunks = search_faiss(query_embedding, index, texts)  # Retrieve context
    context = "\n".join(retrieved_chunks)  # Combine top chunks
    answer = ask_gemini(query, context)  # Get final answer from Gemini
    # print("\n🔹 Context:", context)
    return answer
